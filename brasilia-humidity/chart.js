...
export const addDiagonalsPattern = (svg, scale = 1, colour = '#000000') => {
    let id = 'diagonalPattern' + (svg.attr('id') !== undefined ? svg.attr('id') : Math.random())
...
const addPattern = (type, svg, scale = 1, colour = '#000000') => {
    switch (type) {
        case 'diagonal':
            return addDiagonalsPattern(svg, scale, colour)
        case 'wave':
            return addWavesPattern(svg, scale, colour)
        case 'cross':
            return addCrossPattern(svg, scale, colour)
        case 'triangle':
            return addTrianglePattern(svg, scale, colour)
        case 'scales':
            return addScalesPattern(svg, scale, colour)
